...
import { Calendar, Check, Filter, TrendingDown, TrendingUp, Clock, X, ChevronLeft, ChevronRight, Tag, Repeat, CheckCircle } from 'lucide-react'
import { useFinanceStore } from '@/store/financeStore'
import { format, addDays, isToday, isTomorrow, differenceInDays, startOfMonth, endOfMonth, eachDayOfInterval, isSameMonth, addMonths, subMonths, isSameDay } from 'date-fns'
...
export default function RecurringCalendar() {
  ...
  const [selectedPayment, setSelectedPayment] = useState<{ date: Date; recurring: RecurringMinimal } | null>(null)
  ...
  const upcomingPayments = useMemo(() => {
    const payments: Array<{ date: Date; recurring: any }> = []
  ...
  const totalExpenses = currentPayments.filter(p => p.recurring.type === 'expense')
    .reduce((sum, p) => sum + p.recurring.amount, 0)
  
  const totalIncome = currentPayments.filter(p => p.recurring.type === 'income')
    .reduce((sum, p) => sum + p.recurring.amount, 0)
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400">
                {currentPayments.length} agendamento{currentPayments.length !== 1 ? 's' : ''} encontrado{currentPayments.length !== 1 ? 's' : ''}
              </p>
            </div>
          </div>
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 text-xs text-gray-600 dark:text-gray-400 flex-wrap">
                        ...
                        <span>•</span>
                        <span className={isDayToday || isDayTomorrow ? 'font-semibold text-accent-600 dark:text-accent-400' : ''}>
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <p className="text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase">
                                {isPaid ? '✓ Pagamento Realizado' : selectedPayment.recurring.type === 'expense' ? 'Despesa' : 'Receita'}
                              </p>
                        ...
                        {isPaid ? (
                          ...
                          <div className="p-3 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-xl">
                            <p className="text-xs text-blue-900 dark:text-blue-300 leading-relaxed">
                              <strong>ℹ️ Informação:</strong> Esta é uma transação recorrente {frequencyLabels[selectedPayment.recurring.frequency as RecurrenceFrequency].toLowerCase()}.
