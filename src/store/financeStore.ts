...
import { FinanceState, Transaction, RecurringTransaction, Goal, Budget, SavedMoney, FinancialProjection, SpendingPattern } from '@/types/finance'
import { startOfMonth, format, subMonths, addDays, addWeeks, addMonths, addYears, isAfter, isBefore, getDay } from 'date-fns'

// Helper para converter valores em BRL
  ...
  persist(
    (set, get) => ({
      ...
      addTransaction: (transaction: Omit<Transaction, 'id'>) => 
        set((state) => {
          ...
          const exchangeRate = currency === 'BRL' ? 1 : (transaction.exchangeRate ?? state.currencies[currency] ?? 1)
        ...
        state.recurringTransactions.forEach(recurring => {
          ...
          switch (recurring.frequency) {
            case 'daily':
              ...
              shouldExecute = isBefore(nextDate, now) || nextDate.toDateString() === now.toDateString()
              break
            case 'weekly':
      ...
      setBudget: (category: string, limit: number, period: 'monthly' | 'weekly') =>
        set((state) => {
          const existingBudgets = state.budgets.filter(b => b.category !== category)
          return {
            budgets: [
              ...existingBudgets,
      ...
      getBalance: () => {
        const state = get()
        const totalIncome = state.salary + state.transactions
      ...
      getTotalIncome: () => {
        const state = get()
        return state.salary + state.transactions
          .filter(t => t.type === 'income')
          .reduce((sum, t) => sum + amountInBRL(t, state), 0)
      },
      ...
      getFinancialProjection: (months: number): FinancialProjection[] => {
        ...
        const expenseTrend = monthlyData.length > 1 
          ? (monthlyData[monthlyData.length - 1].expenses - monthlyData[0].expenses) / monthlyData.length
          : 0
        ...
        for (let i = 1; i <= months; i++) {
          ...
          projections.push({
            ...
            projectedIncome,
            projectedExpenses,
            confidence: Math.max(100 - (i * 10), 50)
          })
        }
      ...
      getSpendingPatterns: (): SpendingPattern[] => {
        ...
        const patterns = daysOfWeek.map((dayName, dayIndex) => {
          const dayTransactions = state.transactions.filter(t => 
            t.type === 'expense' && getDay(new Date(t.date)) === dayIndex
          )
        ...
        return patterns.map(p => ({
          ...
          trend: p.averageSpending > totalAvg * 1.2 ? 'increasing' : 
                 p.averageSpending < totalAvg * 0.8 ? 'decreasing' : 'stable'
        }))
      },
      ...
      importData: (data: string) => {
        try {
          ...
          set({
            ...
            recurringTransactions: imported.recurringTransactions || [],
            goals: imported.goals || [],
            budgets: imported.budgets || [],
            savedMoney: imported.savedMoney || [],
            currencies: imported.currencies || { BRL: 1, USD: 5.85, EUR: 6.15 }
          })
        } catch (error) {
    ...
    }),
    {
      ...
      migrate: (persisted: any, version) => {
        ...
        const cur = persisted.state.currencies || { BRL: 1, USD: 5.85, EUR: 6.15 }
        const txs = (persisted.state.transactions || []).map((t: any) => {
          const amount = Number(t.amount)
          const currency = t.currency || 'BRL'
          const rate = currency === 'BRL' ? 1 : (t.exchangeRate ?? cur[currency] ?? 1)
          return { ...t, amount: Number.isFinite(amount) ? amount : 0, currency, exchangeRate: rate }
        })
        const rec = (persisted.state.recurringTransactions || []).map((r: any) => ({
...
// Processar transações recorrentes ao iniciar
